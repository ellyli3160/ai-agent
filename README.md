...
- [x] `sql/004_symbol_track_record_view.sql`（`v_symbol_track_record`）已在
      正式專案執行並用已知案例核對正確（CRWD/LMT/MU/XOM）
- [ ] `n8n/patch_history_check.json` 的兩個節點：已寫好、Code 節點邏輯已用
      模擬資料測過四種情境（正常、查詢結果重複列、查詢結果為空、候選清單
      為空），**尚未貼進正式工作流**，需要你自己操作，步驟見下方「安裝順序」
      第 7 步
...
```
13. 挑選首選標的前，請參考「候選標的歷史推薦紀錄」：根據實際歷史數據，
    重複推薦同一檔股票的達標率（38%）明顯低於首次推薦（62%）。若某標的
    近期已推薦過、且沒有出現新的催化事件，應優先考慮其他候選標的；
    只有在有明確理由（例如題材出現實質性新進展）時才再次推薦同一檔股票，
    並在 reason 欄位裡說明「為何這次不同於上次」
```
